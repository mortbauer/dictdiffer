...
class DictDiffer(object):
    ...
    def _diff_dict(self,first,second,node):
    ...
    def _diff_list(self,first,second,node):
    ...
    def _diff_generic(self,first,second,node):
        """ creates a diff of all the other types not implemented """
        if first != second:
            yield self.CHANGE,self._join_nodes(node), ('generic',first,second)

    def diff(self,first,second,node=[]):
        if type(first) == type(second):
            return getattr(self,'_diff_%s'%type(first).__name__,self._diff_generic)(first,second,node)
        else:
    ...
    def _patch_change_generic(self,destination,node,changes):
    ...
    def _swap_change(self,node, changes):
        return getattr(self,'_swap_change_%s'%changes[0],self._swap_change_generic)(node,changes[1:])

    def _swap_change_generic(self,node, changes):
        first, second = changes
        return self.CHANGE, node, ('generic',second, first)
